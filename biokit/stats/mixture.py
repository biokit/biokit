...
class GaussianModel(Model):
    ...
    def generate(self, N):
        return np.array(
            [scipy.stats.norm.rvs(self.mu, self.sigma) for this in range(N)]
        )
...
class GaussianMixtureFitting(Fitting):
    ...
    def estimate(self, guess=None, k=None, maxfev=2e4, maxiter=1e3, bounds=None):
        ...
        if self.results.log_likelihood and unstable is False:
            ...
            self.results.AICc = criteria.AICc(
                self.results.log_likelihood, self.k, self.data.size, logL=True
            )
            self.results.BIC = criteria.BIC(
                self.results.log_likelihood, self.k, self.data.size, logL=True
            )
        else:
...
class EM(Fitting):
    ...
    def estimate(self, guess=None, k=2):
        ...
        self.results.AICc = criteria.AICc(
            log_likelihood, self.k, self.data.size, logL=True
        )
        self.results.BIC = criteria.BIC(
            log_likelihood, self.k, self.data.size, logL=True
        )
